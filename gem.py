    ...
    try:
        ...
        if image_data:
            ...
            try:
                ...
                image_url = image_url.strip().replace("'", "").replace('"', "")
                ...
                print(f"Image saved at: {filepath}, serving at URL: {image_url}")
...
async def edit_image(
    ...
    try:
        ...
        temp_image_path = os.path.join(TEMP_DIR, f"temp_{uuid.uuid4()}.png")
        with open(temp_image_path, "wb") as buffer:
            shutil.copyfileobj(image.file, buffer)
        
        input_image_filename = f"input_{uuid.uuid4()}.png"
        input_image_path = os.path.join(TEMP_DIR, input_image_filename)
        shutil.copy(temp_image_path, input_image_path)
        input_image_url = f"/images/{input_image_filename}"
        ...
        try:
            ...
            try:
                ...
                orig_filename = f"error_{uuid.uuid4()}.png"
                orig_filepath = os.path.join(TEMP_DIR, orig_filename)
                shutil.copy(temp_image_path, orig_filepath)
                
                return {
                    "success": False,
                    "message": f"Error editing image: {error_message}",
                    "image_url": f"/images/{orig_filename}"
                }
        ...
        if image_data:
            ...
            try:
                ...
                print(f"Edited image saved at: {filepath}, serving at URL: {image_url}")
